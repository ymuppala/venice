...
public class ControlMessageDumperTest {
  ...
  @Test
  public void testDisplayKeepsExistingEndOfSegmentOutput() {
    GUID producerGuid = new GUID();
    DefaultPubSubMessage eos = buildEndOfSegment(producerGuid, true, new byte[] { 1, 2, 3 }, 200L);

    String output = runDumperAndCaptureOutput(Collections.singletonList(eos), false);

    assertTrue(output.contains("END_OF_SEGMENT"), output);
    assertTrue(output.contains("is final segment: true"), output);
    assertTrue(output.contains("check sum: " + Arrays.toString(new byte[] { 1, 2, 3 })), output);
    assertFalse(output.contains("headers:"), "EOS without headers should not print headers: block: " + output);
  }

  @Test
  public void testDisplayReturnsTotalControlMessageCount() {
    GUID producer1 = new GUID();
    GUID producer2 = new GUID();
    PubSubMessageHeaders headers = new PubSubMessageHeaders().add(
        PubSubMessageHeaders.VENICE_PARTITION_RECORD_COUNT_HEADER,
        ByteBuffer.allocate(Long.BYTES).putLong(7).array());

    DefaultPubSubMessage eopP1 = buildControlMessage(producer1, ControlMessageType.END_OF_PUSH, headers, 1L);
    DefaultPubSubMessage eosP1 = buildEndOfSegment(producer1, true, new byte[] {}, 2L);
    DefaultPubSubMessage eopP2 = buildControlMessage(producer2, ControlMessageType.END_OF_PUSH, null, 3L);

    PubSubConsumerAdapter consumer = mockConsumer(Arrays.asList(eopP1, eosP1, eopP2));
    ControlMessageDumper dumper =
        new ControlMessageDumper(consumer, TOPIC, PARTITION, ApacheKafkaOffsetPosition.of(0), 10);

    int total = dumper.fetch().display();

    assertEquals(total, 3, "All three control messages should be counted");
  }
  ...
  private static DefaultPubSubMessage buildEndOfSegment(
      GUID producerGuid,
    ...
    EndOfSegment eos = new EndOfSegment();
    eos.finalSegment = finalSegment;
    eos.checksumValue = ByteBuffer.wrap(checksum);
    eos.computedAggregates = Collections.emptyList();
    controlMessage.controlMessageUnion = eos;
    return wrap(producerGuid, controlMessage, null, offset);
  }
