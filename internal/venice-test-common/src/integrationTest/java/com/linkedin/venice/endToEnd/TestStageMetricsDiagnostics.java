...
public class TestStageMetricsDiagnostics extends AbstractMultiRegionTest {
  ...
  public void testAllStageMetrics() throws Exception {
    ...
    try (VenicePushJob repushJob = new VenicePushJob("diagnostics-all-stages-repush", repushProps)) {
      ...
      // Use >= instead of assertEquals: Spark accumulators have at-least-once semantics under
      // task retries/speculation, so cross-stage totals can legitimately over-count by different
      // amounts even when the pipeline is correct. The >= relationship still validates data flow
      // ordering while tolerating over-counts.
      assertTrue(
          compressIn >= compactOut,
          "Compression recordsIn (" + compressIn + ") should be >= compaction recordsOut (" + compactOut + ")");
      assertTrue(
          compressOut >= compressIn,
          "Compression recordsOut (" + compressOut + ") should be >= recordsIn (" + compressIn + ")");

      // Byte consistency: compression input bytes should be >= compaction output bytes
      // (same records flow from compaction to compression; >= for accumulator at-least-once semantics)
      long compressBytesIn = compression.getBytesIn();
      long compressBytesOut = compression.getBytesOut();
      assertTrue(
          compressBytesIn >= compactBytesOut,
          "Compression bytesIn (" + compressBytesIn + ") should be >= compaction bytesOut (" + compactBytesOut + ")");
      ...
      // Kafka write input bytes should be >= compression output bytes
      // (equal in practice, but >= accounts for accumulator at-least-once semantics under retries)
      long kwBytesIn = kafkaWrite.getBytesIn();
      assertTrue(
          kwBytesIn >= compressBytesOut,
          "Kafka write bytesIn (" + kwBytesIn + ") should be >= compression bytesOut (" + compressBytesOut + ")");
  ...
  public void testChunkedStageMetrics() throws Exception {
    ...
    try (VenicePushJob repushJob = new VenicePushJob("diagnostics-chunked-repush", repushProps)) {
      ...
      assertTrue(
          chunkAssembly.getRecordsOut() >= recordCount,
          "Chunk assembly should output at least " + recordCount + " assembled records; actual="
